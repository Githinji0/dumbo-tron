...
async def run_test():
    ...
    async with db_factory() as db:
        ...
        async with httpx.AsyncClient(follow_redirects=True, headers=HEADERS, timeout=60.0) as client:
            ...
            probes = [
                {"search": "close", "limit": 2},
                {"dataset.id": "fundamental6", "delay": 1, "instrumentType": "EQUITY", "region": "USA", "universe": "TOP3000", "limit": 2},
                {"dataset": "fundamental6", "delay": 1, "instrumentType": "EQUITY", "region": "USA", "universe": "TOP3000", "limit": 2},
                {"dataset.id": "fundamental6", "delay": 1, "region": "USA", "universe": "TOP3000", "limit": 2},
            ]
            
            for p in probes:
                print(f"\nGET /data-fields with {p}")
                r = await client.get("https://api.worldquantbrain.com/data-fields", params=p)
                print(f"Status Code: {r.status_code}")
                if r.status_code == 200:
                    print(f"Success! Count: {len(r.json().get('results', []))}")
                    break
                else:

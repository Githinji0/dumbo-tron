...
async def run_test():
    ...
    async with db_factory() as db:
        ...
        async with httpx.AsyncClient(follow_redirects=True, headers=HEADERS, timeout=60.0) as client:
            ...
            url = "https://api.worldquantbrain.com/datasets"
            # Optional query params: region, delay, universe, instrumentType
            params = {"region": "USA", "delay": 1, "universe": "TOP3000", "instrumentType": "EQUITY", "limit": 10}
            ...
            if r.status_code == 200:
                ...
                for idx, ds in enumerate(data.get('results', [])[:5]):
                    print(f"  [{idx}] ID: {ds.get('id')}, Name: {ds.get('name')}")
            else:
